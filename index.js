...
const authorize = (allowedRoles) => {
  return (req, res, next) => {
    ...
    if (allowedRoles.includes(userRole)) {
      ...
      res.status(403).json({ message: 'Acceso denegado: no tienes los permisos necesarios.' }); // El rol no tiene permiso
    }
  };
};
...
app.post('/api/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user.id, email: user.email, codigo: user.codigo, role: user.role },
...
app.post(
    ...
    async (req, res) => {
        ...
        try {
            await pool.query('INSERT INTO affiliations (user_id, form_data) VALUES ($1, $2)', [userId, formData]);
